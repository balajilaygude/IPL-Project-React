...
export default function PointsTable() {
  ...
  return (
    <div className='w-full p-5 flex justify-center items-center'>
      ...
      <table className='w-full sm:w-4/5 p-2 text-center'>
        <tr>
          <th className='w-2/6 text-left '>Team</th>
          <th>M</th>
          <th>W</th>
          <th>L</th>
          <th>D</th>
      ...
      {
        pointsTableData.pointsTable[0].pointsTableInfo.map(({teamId,form,teamName,matchesPlayed,matchesWon,matchesLost,noRes,points,teamImageId},i)=>
            <tr key={teamId} className='border-b-2 border-gray-300 '>
              <div className=' flex gap-2'>
            <td className='sm:w-6'>{i+1}</td>
            <td><img className='w-4 h-4'src={`https://res.cloudinary.com/digkgdovw/image/upload/v1715267905/iplTeamLogo/${teamImageId}`} alt="" /></td>  
            <td> {teamName}</td>
              </div>
            <td> {matchesPlayed}</td>
            <td> {matchesWon}</td>
            ...
            <td> {noRes ? noRes: "-"}</td>
            <td> {points}</td>
            <td> {form.reverse().map(data=>data==="W"?<i className="fi fi-ss-check-circle text-sm text-green-500"></i>:
            <i class="fi fi-sr-cross-circle text-sm text-red-500"></i>)}</td>
